...
- **Better handed off**: what it is, why it meets all three tests, and that you'd produce a scoped `/handoff` for it.
  Say plainly that they can decline and have it finished in-session or captured durably instead — a handoff is a proposal about how to move work, not a verdict on it.
...
- **A handoff they accepted**: produce it in scoped mode — the thread only, not the session — and then treat it as *handed off, unconfirmed* for the rest of this session.
