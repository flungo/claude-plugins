...
**Split plugins by enablement boundary, not by topic.** Two things always enabled together at the same scope can be one plugin (as separate skills); two things enabled at different scopes must be different plugins.
...
**Compose plugins via first-party marketplace dependencies.** Where a plugin builds on another, it declares the dependency in its `.claude-plugin/plugin.json` `dependencies` array (bare string = latest in this marketplace).
