...
- **Tool version — read it from a CI run, don't trust this note.**
  ...
  Last seen: **0.23.2** (markdownlint 0.41.1), 2026-09-12.
...
- A reference locates a script its plugin ships by a path relative to itself, because `${CLAUDE_PLUGIN_ROOT}` resolves in skill content but not in a reference read from disk nor in the Bash tool's environment; `SKILL.md` names the runnable path, and nothing searches the plugin cache ([ADR-010](docs/decisions/010-locating-a-shipped-script.md)).
