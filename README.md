...
- **[claude-code-web](plugins/claude-code-web)** — always-on working preferences for Claude Code Web (user scope): the egress proxy and CA bundle, the GitHub MCP as the *only* route to the API there, containers replaced across idle periods, repo scoping and `add_repo`'s cross-owner rule, project config in a multi-repo session, why repo-adopted plugins never load and how the user-scope ones arrive, starting another session from inside one and what it does and doesn't inherit, the PR-subscription boilerplate's standing check-in request and why it is declined, and delegating unrunnable steps to CI.
...
- **[writing-styles](plugins/writing-styles)** — a library of prose styles, one reference file per style: currently **instructional-writing**, for text a reader acts on as instruction rather than reads as background, wherever it lives.
  State the current truth rather than the document's own history, converge on plain fact over time, fix wrong guidance at its source, and never direct an agent to do what only the user can do.
  Declared as a dependency by `claude-plugin-standards` and `docs-standards`, so the rules are stated once and cited by name.
  It applies nothing by being enabled — a style takes effect only where something names it — which is what makes it safe as a dependency at either scope.
- **[claude-plugin-standards](plugins/claude-plugin-standards)** — repo-adopted conventions (project scope) for authoring plugins: the directory and manifest layout, composing via first-party dependencies, declaring every dependency you reference, citing a dependency's skill or reference by name rather than by path, whether a plugin is ambient or on-demand and what may therefore depend on it, filing a fact by what it is a property of, skill naming in single- and multi-skill plugins, keeping cross-references current by basename, the reserved word that makes a skill silently fail to load on claude.ai, `SKILL.md` frontmatter hazards, editing a JSON manifest without mangling it, validating and test-installing before committing, and the minor-versus-patch test.
- **[session-workflow](plugins/session-workflow)** — personal, always-on commands (user scope) for ending a Claude session well: `/session-clean` (aliases "Session Clean?", "Safe to delete?")
  checks whether closing the session would lose anything and proposes where each loose end should be recorded, and `/handoff` produces a document that carries unfinished work into a fresh session — and, where the session can start another session, offers to open the receiving one from it directly.
  The two halves of the same moment — record it, or carry it.
  Depends on `connector-conventions`, since filing an issue means reading existing ones through the GitHub MCP first, and on `delegation-conventions` for the model a session it starts should run on.
