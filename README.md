...
- **[claude-code-web](plugins/claude-code-web)** — always-on working preferences for Claude Code Web (user scope): the egress proxy and CA bundle, the GitHub MCP as the *only* route to the API there, containers replaced across idle periods, repo scoping and `add_repo`'s cross-owner rule, project config in a multi-repo session, why repo-adopted plugins never load and how the user-scope ones arrive, starting another session from inside one and what it does and doesn't inherit, and delegating unrunnable steps to CI.
...
- **[writing-styles](plugins/writing-styles)** — a library of prose styles, one reference file per style: currently **instructional-writing**, for text a reader acts on as instruction (a `SKILL.md` and its references, a Diátaxis reference doc, a runbook).
